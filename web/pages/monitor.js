...
function mostrarAlertaFlutuante(ev) {
    ...
    alerta.innerHTML = `
        ...
        <div style="font-size:13px;font-weight:700;color:var(--text);line-height:1.5;margin-bottom:6px;">
        ...
        <div style="font-size:10px;font-weight:700;color:var(--text3);margin-top:8px;letter-spacing:1px;">
...
function exibirDicaMonitor(dica, tipo) {
    ...
    painel.innerHTML = `
        ...
        <div style="font-size:13px;font-weight:700;color:var(--text);line-height:1.6;">
            ${esc(dica).replace(/\n/g, '<br>')}
        </div>
        <div style="margin-top:12px;display:flex;gap:8px;">
            <button class="btn btn-accent" style="font-size:11px;padding:6px 14px;font-weight:700;"
                    onclick="navegarPara(window.PG.MONITOR);document.getElementById('dicaFlutuante').remove();">
                VER MONITOR
            </button>
            <button class="btn btn-ghost" style="font-size:11px;padding:6px 14px;font-weight:700;"
...
function pgMonitor(wrap) {
    ...
    wrap.innerHTML = `
        <div class="page-header" style="margin-bottom:16px;">
            <div>
                <div class="page-title" style="font-weight:700;">MONITORAMENTO INTELIGENTE</div>
                <div class="page-sub" style="font-weight:700;">Visão computacional em tempo real via Qwen VL</div>
            </div>
            <div style="display:flex;gap:10px;align-items:center;">
                <div id="monitorBadge" style="font-family:var(--mono);font-size:14px;font-weight:700;
                ...
                <button class="btn btn-accent" id="btnToggleMonitor"
                        onclick="toggleMonitor()" style="min-width:120px;font-weight:700;">
                    ${m.ativo ? '⏹ PARAR' : '▶ INICIAR'}
                </button>
                <select id="monitorIntervalo" class="input" style="width:110px;padding:8px 10px;font-weight:700;"
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:14px;">
            <div class="card" style="padding:18px;">
                ...
                <div id="monitorUltima" style="font-size:13px;font-weight:700;color:var(--text);line-height:1.6;min-height:60px;">
                    ${m.ultimo_resumo
            ...
            <div class="card" style="padding:18px;">
                ...
                <div id="monitorUltimaDica" style="font-size:13px;font-weight:700;color:var(--text);line-height:1.6;min-height:60px;">
                    ${m.ultima_dica
        ...
        <div class="card" style="padding:18px;flex:1;">
            ...
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;margin-top:4px;">
                <div style="font-family:var(--mono);font-size:14px;font-weight:700;
                     color:var(--text3);letter-spacing:3px;">HISTÓRICO DE ALERTAS</div>
                <button class="btn btn-ghost" style="font-size:11px;padding:5px 12px;font-weight:700;"
                        onclick="limparEventosMonitor()">🗑 LIMPAR</button>
            </div>
    ...
    return state.monitor.eventos.map(ev => {
        ...
        return `
            <div style="padding:12px 14px;margin-bottom:8px;border-radius:8px;
                border:1px solid ${cor}33;background:${cor}08;border-left:3px solid ${cor};">
                <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;">
                    ...
                    <span style="margin-left:auto;font-family:var(--mono);font-size:10px;
                          font-weight:700;color:var(--text3);">${ev.ts}</span>
                </div>
                <div style="font-size:13px;font-weight:700;color:var(--text);margin-bottom:4px;">
                    ${esc(ev.resumo)}
                </div>
                ${ev.sugestao ? `
                <div style="font-size:12px;color:${cor};font-weight:700;margin-top:4px;">
                    → ${esc(ev.sugestao)}
                </div>` : ''}
            </div>`;
    }).join('');
...
function atualizarHeaderMonitor() {
    ...
    if (btn) btn.textContent = m.ativo ? '⏹ PARAR' : '▶ INICIAR';
...
function toggleMonitor() {
    ...
    if (state.page === PG.MONITOR) {
        setTimeout(() => { if (state.page === PG.MONITOR) renderPage(); }, 300);
    }
}

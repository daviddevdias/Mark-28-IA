...
function metricCard(lbl, idV, idP, val, pct, cor) {
    ...
    return `
        <div class="metric-card">
            ...
            <div class="metric-label" style="margin-top:8px;font-weight:700;">${lbl}</div>
            <div class="metric-val" id="${idV}" style="color:${col};font-weight:700;">${val}</div>
...
function spec(k, v) {
    return `
        <div class="spec-block">
            <div class="spec-label" style="font-weight:700;">${k}</div>
            <div class="spec-val"   style="font-weight:700;">${v}</div>
        </div>`;
}
...
function quickBtn(icon, label, cmd, col, bg) {
    return `
        <div class="quick-btn" style="--hover-col:${bg};color:${col};border-color:var(--border);"
            ...
            <span class="quick-label" style="color:${col};font-weight:700;">${label}</span>
        </div>`;
}
...
function updateMetrics() {
    ...
    const gpuC = m.gpu > 85 ? 'var(--red)' : 'var(--orange)';

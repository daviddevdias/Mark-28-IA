...
function pgNotas(wrap) {
        ...
        <div class="chat-wrap" style="max-height: 500px; display: flex; flex-direction: column;">
            ...
            <div class="chat-input-row" style="gap: 8px; padding: 10px; border-top: 1px solid var(--border);">
                <div style="flex: 1; display: flex; align-items: center; gap: 10px;">
                    <div class="status-glitch" id="notasStatus" style="font-family: var(--mono); font-size: 14ppx; color: var(--accent); letter-spacing: 1px;">0 CHR</div>
                </div>
                <button class="btn btn-ghost" onclick="limparNotas()" style="padding: 6px 15px; font-size: 11px;">LIMPAR</button>
                <button class="btn btn-accent" onclick="salvarNotas()" style="padding: 6px 15px; font-size: 11px;">SALVAR</button>
            </div>
        </div>`;
       ...
       if (ta) {
              const atualizarMeta = (val) => {
                     ...
                     document.getElementById('notasStatus').textContent = `${chars} CHR`;
              };
...
function salvarNotas() {
       if (window.jarvis) {
              window.jarvis.salvar_configuracao('notas', state.notas);
       }

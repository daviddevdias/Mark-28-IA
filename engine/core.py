...
async def analisar_camera_agora() -> None:
    ...
    raw = await chamar_qwen(SYSTEM_RAPIDO, "Analise esta imagem da camara: codigo no ecra, erros, ou o que ve.", img, 180)
    resultado = parse(raw, img)
    emitir({
        "visao_img": img,
        "visao_resultado": resultado.resumo,
...
async def desligar_monitoramento() -> None:
    ...
    print(f"[SISTEMA] Monitor desligado — problemas: {stats.get('total_problemas', 0)}")


async def status_do_sistema() -> None:
    s = info_monitor()
    if s["rodando"]:
        msg = f"Operacional. {s['chamadas_api']} consultas, {s.get('total_problemas', 0)} problema(s)."
    else:
...
async def processar_comando(comando: str, imagem_monitor: Optional[Any] = None) -> str | None:
    ...
    if monitor_state.aguardando_confirmacao:
        ...
        aceitar = (
            "pedido aceito", "pode ajudar", "pode analisar", "pode resolver",
            "pode continuar", "continua com a ajuda", "segue com a ajuda",
            "aceito a ajuda", "aceito", "confirmado",
        )
        recusar = (
            "dispensa ajuda", "não precisa de ajuda", "nao precisa de ajuda",
            "ignora isso", "ignorar isso", "pode ignorar",
            "agora não", "agora nao", "depois", "cancelar ajuda",
        )

        if any(p in cmd for p in aceitar):
            set_aguardando(False)
        ...
        if any(p in cmd for p in recusar):
            set_aguardando(False)
            msg = "Entendido. Monitoramento continua."
            await falar(msg)
            return msg

        msg = "Ainda aguardo confirmação. Diga: 'pedido aceito, pode ajudar' ou 'dispensa ajuda'."
        await falar(msg)
        return msg
        ...
        if resultado:
            ...
            if OBS:
                try:
                    ...
                    registrar_acao("comando_local", descricao=comando[:200], modulo="controller", duracao_ms=dur, sucesso=True)
    ...
    if resposta:
        ...
        if OBS:
            try:
                dur = int((time.time() - ts_inicio) * 1000)
                registrar_acao("comando_ia", descricao=comando[:200], modulo="ia_router", duracao_ms=dur, sucesso=True)
                registrar_metrica("cmd.duracao_ms", dur, "ms")
